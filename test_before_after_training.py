    ...
    with torch.no_grad():
        for images, masks in dataloader:
            ...
            for i in range(images.shape[0]):
                ...
                # The output mask is expected to be of the same size as the input image
                pred_masks = processor.image_processor.post_process_masks(
                    outputs.pred_masks.cpu(), [(1024, 1024)], [(1024, 1024)]
                )
                pred_mask = pred_masks[0].squeeze(0).squeeze(0).cpu().numpy()
                # Resize the ground truth mask to match the size of the predicted mask
                resized_gt_mask = torch.nn.functional.interpolate(input_mask, size=(1024, 1024), mode='nearest').squeeze(0).cpu().numpy()
                iou = calculate_metrics(pred_mask, resized_gt_mask)
                total_iou += iou
                iou_count += 1
